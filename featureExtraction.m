function [ DCTs ] = featureExtraction( filename, frameTimeLength, HMMtype)
...
%      frameTimeLength = time of each short time frame.
    ...
    [speechFile,Fs] = audioread(filename);                            % Import the Audio Signal.
    speechFile = speechFile(:,1);                                   % Since the Audio Signal is recorded in 2 channels, it is trimmed to single channel.
   % noiseFile = resample(wavread(noiseName), 8000, 16000) ;         %DownSample noiseFile
   % speechFile(1:10000) = speechFile(1:10000) + noiseFile(1:10000)/1000; %Getting Rid of the Zeros in Speech
    
    speechFile = speechFile + 0.01;
    ...
    frameShift = frameSize * 0.5;                                   % overlapping (10ms)
  ...
  %-------------------------------------------------------------------------------------------%  
    for frame = 1 : numFrames,
         [ magnitude, energy ] = SpeechProcess(frame, frameSizeSample, frameShiftSample, hamWindow, speechFile);
         
        for nFilter = 1: numFilters
%             [filteredResult] = myFilter( filterLength, magnitude, numFilters, nFilter );
           [filteredResult] = myMelFilter(myMel, magnitude, nFilter );
...
%         HTKwriteMFC( nSamples, sampPeriod, sampSize, parmKind, vecSize, vDCTs, mfcName );
    ...
    elseif (HMMtype == 4)                                       % create MFCC_E_D_A
      ...
      %  HTKwriteMFC( nSamples, sampPeriod, sampSize, parmKind, DCTs, vaDCTs, mfcName );
